...
class PipeResultFaceTransform(BaseModel):
...
class PipeFaceTransform(PipeBase):
    ...
    def forward(self, inputs: List[np.ndarray], inference_cfg: InferenceConfig) -> List[np.ndarray]:
        ...
        if face_transform:
            for image in inputs:
                ...
                face_predictions = self.face_detector(image, **inference_cfg.face_detector.dump_model())
                ...
                for prediction in face_predictions:
                    ...
                    image[y : y + h, x : x + w] = face_transformed
